...
function loadGoodtype(jsonEntity){
  //方法级渲染
  table.render({
   elem: '#listTable'
   ,url: '/goodtype/listByPramas'
   ,width:1560
   ,height: 480
   ,page:true
   ,even:true
   ...
   ,cols: [[
     {checkbox: true}
     ,{field:'tid', title: 'ID',align:'center', width:80, sort: true}
     ,{field:'tname', title: '类型名称',align:'center', width:160, sort: true}
     ,{field:'tdesc', title: '类型简介',align:'center', width:160, sort: true}
  ...
  table.on('tool(list)', function(obj){
    ...
    if(obj.event === 'show'){
      ...
      layer.confirm('真的删除行么', function(index){
        var jsonEntity = {};
        jsonEntity['tid'] = data.tid;
        czGoodtype("/goodtype/delete",jsonEntity,obj);
        layer.close(index);
      });
      ...
      layer.open({
		...
		anim:4,
        shade:0.6,
  		content:$("#updGoodtypeDiv"),
  		success:function(){
        form.val("updGoodtypeForm", {
    "tname": data.tname,
    "tdesc": data.tdesc,
        });

  		          //监听提交
			  form.on('submit(submitUpdGoodtype)', function(FormData){
			    var jsonEntity = FormData.field
			    jsonEntity.tid = data.tid;
			    czGoodtype("/goodtype/updT",jsonEntity,obj);
			    layer.closeAll();
			    return false;
			  });
  		}
    ...
    $("#delBatchGoodtype").click(function(){
	   ...
	   if(data.length<1){
          layer.msg("请选择数据！！！",{"icon":3,anim:6,shade:0.6,"time":2000});
           ...
           layer.confirm("你确定要删除所选中的数据吗？？？", {icon: 7, title: '提示'}, function (index) {
               ...
               delManyGoodtype("/goodtype/deletes", params);
               flush(currentPage);
               layer.close(index);
           });
       }
  ...
  $("#saveUI").click(function(){
	  layer.open({
	    ...
	    shade:0.6,
		  ...
		  form.on('submit(submitUpdGoodtype)', function(FormData){
            var jsonEntity = FormData.field
		    czGoodtype("/goodtype/saveT",jsonEntity);
		    layer.closeAll();
		    return false;
		  });
  		}
  ...
  //普通增删改操作
  function czGoodtype(url,jsonEntity,obj){
	  $.ajax({
		   	type:"post",
		   	url:url,
		   	data:jsonEntity,
		   	success:function(data){
		   		if(data=="delSuccess"){
	   				...
	   				layer.msg("信息修改成功。。。",{"icon":6,anim:4,shade:0.6,"time":2000});
		   			 obj.update({
                                                                                                                tname: jsonEntity.tname,
                                                                                         tdesc: jsonEntity.tdesc,
                                           		   	      });
  ...
  //批量删除
  function delManyGoodtype(url,ids){
	  $.ajax({
			type:"post",
			url:url,
			data:{'ids':ids},
